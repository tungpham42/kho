...
<div x-data="{ showModal: false }" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
        <div class="relative w-full sm:w-80">
            ...
            <input type="text" placeholder="Tìm kiếm mã kho, tên kho..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all outline-none placeholder-slate-400">
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    <td colspan="5" class="p-12 text-center">
                        ...
                        <p class="text-slate-500 font-medium">Bạn chưa cấu hình kho bãi nào.</p>
                        <button @click="showModal = true" class="text-indigo-600 font-bold hover:underline mt-2 inline-block">Thêm kho bãi đầu tiên</button>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div x-cloak x-show="showModal"
        ...
        <div @click.away="showModal = false"
             ...
             class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden border border-slate-100">
            ...
            <form action="{{ route('warehouses.store') }}" method="POST" class="p-6 sm:p-8 space-y-5">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="address" rows="2" placeholder="Nhập địa chỉ chi tiết..." class="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all outline-none placeholder-slate-400 resize-none"></textarea>
                </div>
