...
@section('content')
@php
    // Khởi tạo các biến kiểm tra quyền cho màn hình Dashboard
    ...
    // Lấy tên doanh nghiệp an toàn cho cả Tenant và Employee
    $companyName = $isTenant
        ? ($currentUser->company_name ?? 'doanh nghiệp')
        : ($currentUser->tenant->company_name ?? 'doanh nghiệp');
@endphp
...
<div class="space-y-6">
    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-lg p-8 flex justify-between items-center text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h2 class="text-3xl font-extrabold tracking-tight mb-2">Xin chào, {{ $currentUser->name }}! 👋</h2>
            <p class="text-indigo-100 text-lg font-medium">Chào mừng bạn trở lại không gian quản lý kho của <span class="text-white font-black">{{ $companyName }}</span>.</p>
        </div>
